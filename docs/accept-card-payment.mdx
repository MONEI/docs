...
The submitted form should include `paymentToken` generated with Card Input Component. You can also provide additional parameters like `customer.email` (use `.` as a separator for nested parameters). Check all available [parameters](/api/confirm-payment/).
...
:::note
As an alternative process you can submit generated `paymentToken` to your sever and then [confirm payment](/api/confirm-payment/) on the server-side.
:::

### 4. Customer is redirected back to your server.

A customer is redirected to the `completeUrl` with **payment_id** query parameter. Use [get payment](/api/get-payment/) endpoint to get the payment status.
