...
To complete the payment you need to submit a payment form with the `method="post"` and `action="https://secure.monei.net/payments/{{payment_id}}/submit"` (replace `{{payment_id}}` with your payment ID obtained in [step 1](#1-create-a-payment-server-side))
