...
By default, Bizum transactions are capped at 1.500 EUR, with any amount exceeding this limit automatically rejected. In certain cases, this limit can be increased to 15.000 EUR upon specific approval from the MONEI risk and compliance team.

Unlike Bizum's peer-to-peer (P2P) limits, there is no monthly cap on the number of online purchases a customer can make using Bizum.
...
:::note
You can [send a payment request](/api/#operation/payments_send_request) directly to the customer’s phone. If the Bizum payment method is available and the phone number is registered with Bizum, the customer will receive a push notification to confirm the payment directly in their banking app. If not, they’ll receive a payment link via WhatsApp. This feature works independently of the UI Component.
:::
...
:::caution
Bizum is transitioning to a new payment request flow, known as RTP (Request to Pay), which sends a push notification directly to the banking app, allowing payment confirmation without a redirect. Although not yet fully supported by all Spanish banks, this flow already covers nearly 98% of Bizum transactions. To confirm if a redirect is needed, it’s essential to check the `mustRedirect` flag in the response.
:::
