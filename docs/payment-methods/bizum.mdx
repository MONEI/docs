...
Bizum transactions do not have a maximum limit. However, if a transaction fails, it could be due to the issuing bank having its own maximum limit or the consumer lacking sufficient funds. Unlike Bizum's peer-to-peer (P2P) limits, there is no monthly cap on the number of online purchases a customer can make using Bizum.
