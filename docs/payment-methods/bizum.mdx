...
Bizum also has the possibility of carrying out the authentication and the capture of the operation separately. The difference with the standard Bizum Payment is in the type of transaction `AUTH`. Once the authentication is done, you have a maximum of 30 days to capture the operation. After that time, the authorization will not be effective. It can only be used through the RTP process.
