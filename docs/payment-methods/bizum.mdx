...
Bizum transactions can be refunded within a timeframe of max 365 after the initial transaction was processed. If you want to refund a Bizum transaction which is older than 365 days, the same will fail.
...
:::note
Currently Bizum only supports redirect flow.

**It is not possible to confirm payments using monei.js** [confirmPayment](monei-js/reference.md#moneiconfirmpayment-function) function. As an alternative process, you can submit the generated `paymentToken` to your sever and then [confirm the payment](/api/#operation/payments_confirm) on the server-side.

Bizum is implementing a new payment request flow with the push notification to the banking app that will allow confirming the payment without the redirect. This is what's known as RTP (Request to Pay.)
The list of banks that completed the integration is: **Caixabank, BBVA, Grupo Cajamar, Banco Mediolanum, Orange Bank, Laboral Kutxa, Targobank, Banca Pueyo, Banco Caminos, Caixa Ontinyent**
:::
