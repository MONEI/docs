...
You can start accepting Apple Pay payments on the Web using [Hosted Payment Page](integrations/use-prebuilt-payment-page.mdx) or [PaymentRequest UI Component](monei-js/reference.md#moneigooglepay-component). No additional configuration is required to use Apple Pay in Hosted Payment Page.

Our [PaymentRequest UI Component](monei-js/reference.md#moneigooglepay-component) conditionally renders Apple Pay or Google Pay to your payment page depending on a customer browser and operating system. If Apple Pay is supported the component will render Apple Pay button, otherwise it will render Google Pay. If a customer selects a card that requires 3D Secure confirmation the payment confirmation step will automatically show a 3D Secure confirmation popup.
...
:::note
Apple Pay with the PaymentRequest UI Component requires macOS 10.12.1+ or iOS 10.1+.
:::
...
- You can check the status of a Google Pay test payment in your [MONEI Dashboard → Payments](https://dashboard.monei.com/payments) (in test mode).
...
Add MONEI [Google Pay Component](monei-js/reference.md#moneigooglepay-component) to your payment page. Create empty DOM node (container) with unique ID in your payment form.
...
Check [MONEI JS Reference](monei-js/reference.md#moneigooglepay-component) for more options.
