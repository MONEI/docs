...
Use MONEI's `merchantId` from the [Get PaymentMethods](/docs/apis/rest/payment-methods-get.api.mdx) API (`metadata.googlePay.merchantId`). No separate registration with Google is required.
...
Create a payment request with MONEI as the gateway. Use the `merchantId` from the [Get PaymentMethods](/docs/apis/rest/payment-methods-get.api.mdx) API (`metadata.googlePay.merchantId`). No separate registration with Google is required:
...
- Make sure that you have Google Pay enabled in [MONEI Dashboard → Settings → Payment Methods](https://dashboard.monei.com/settings/payment-methods).
- For direct API integration, use the `merchantId` from the [Get PaymentMethods](/docs/apis/rest/payment-methods-get.api.mdx) API.
