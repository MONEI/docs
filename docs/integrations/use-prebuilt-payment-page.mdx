...
- in any other case the customer is redirected to the `completeUrl` with **payment_id** query parameter. Use [get payment](/docs/apis/rest/payments-get/) endpoint to get the payment status.

### 5. An asynchronous request is sent to your server

MONEI will notify you about the payment status by sending an HTTP POST request to the `callbackUrl`. The request body will contain the full [payment object](/docs/apis/rest/schemas/payment/) in JSON format.
