...
|                 | Permanent (No-code)                        | One-Time (API)                                      |
| --------------- | ------------------------------------------ | --------------------------------------------------- |
| **Created via** | Dashboard / MONEI Pay App                  | Payments API                                        |
...
| **Amount**      | Customer enters (manual) or fixed          | Pre-set in API call                                 |
| **Expiration**  | Never (can be disabled)                    | 5 minutes (or custom `expireAt`)                    |
| **Best for**    | Static displays, printed materials, tables | Dynamic checkout, invoices, kiosks                  |
...
- **storeId** `string`: Associate payment with a specific store
- **pointOfSaleId** `string`: Link payment to a dashboard-configured POS
