...
Stores and Points of Sale can be used to control user access and group payments:

- **Assign users to stores/POS**: In [Dashboard → Settings → Users](https://dashboard.monei.com/settings/users), bind specific users (e.g., waiters, cashiers) to a store or POS
- **Restricted visibility**: Bound users only see payments from their assigned store/POS
- **Automatic inheritance**: When a bound user creates a payment via Dashboard or MONEI Pay, the payment automatically inherits their store/POS ID
- **Traceability**: Admins can track which store or POS originated each payment

This enables merchants to give staff limited access while maintaining full oversight.
