...
You can use monei.js’ APIs to integrate prebuilt UI Components for different payment methods into your own checkout flows across desktop and mobile.
...
## `CardInput` UI Component

CardInput is a customizable UI Component used to collect sensitive credit card information in your payment forms.
...
- **style** `object` - Customize the appearance of this component using CSS properties passed in a [Style](#cardinput-style-object) object.
  The [ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) code represents the desired button language. Supported locales include `en`, `es`, `ca`, `pt`, `de`, `it`, `fr`, `nl`, `et`, `fi`, `lv`, `no`, `pl` and `ru`. By default, MONEI smartly detects the correct language for the buyer based on their geolocation and browser preferences.
...
UI components are styled using a Style object. It consists of CSS properties nested under objects for any of the following variants:
...
:::note
Payment tokens generated by monei.js UI Components expire as soon as they are used or 5 days after creation.
:::
...
PayPal is a customizable UI Component that renders a PayPal payment button.
...
## `GooglePay` UI Component

GooglePay is a customizable UI Component that renders a Google Pay payment button.
...
PaymentRequest is a customizable UI Component that renders a Google Pay or Apple Pay payment button depending on the browser and operating system of the user.
...
Bizum is a customizable UI Component that renders a Bizum payment button.
...
After you generate the **paymentToken** using one of the UI Components, use this function to confirm the payment passing the obtained `paymentToken`. It will automatically show a popup window with a 3D Secure confirmation screen if required.
