...
Check all available [request parameters](/api/).
...
- in any other case a customer is redirected to the `completeUrl` with **payment_id** query parameter. Use [get payment](/api/get-payment/) endpoint to get the payment status.

### 5. An asynchronous request is sent to your server.

MONEI will notify you about a payment status by sending an HTTP POST request to the `callbackUrl`. The request body will contain full [payment object](/api/get-payment/) in JSON format.
