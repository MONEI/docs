...
```json
{
  ...
  "nextAction": {
    "type": "PAYMENT_PAGE",
    "redirectUrl": "https://secure.monei.net/pay/af6029f80f5fc73a8ad2753eea0b1be0"
  },
...
- **nextAction** `object` - If present, this property tells you what actions you need to take in order for your customer to fulfill a payment using the provided source.
  - **type** `string` - Next action type, one of:
    - `PAYMENT_PAGE` - Your customer needs to be redirected to a hosted payment page
...
To payment a credit card or other payment method, you create a Payment.
...
- **amount** `required` `positive integer` - Amount intended to be collected by this payment. A positive integer representing how much to payment in the smallest currency unit (e.g., 100 cents to charge 1.00 USD)
