...
Check all available [request parameters](/docs/apis/rest/subscriptions-create/).
...
Check all available [request parameters](/docs/apis/rest/subscriptions-activate/).

### 3. Redirect the customer to the `redirectUrl` from the response

In the response from the [activate subscription](/docs/apis/rest/subscriptions-activate/) request you'll get the following response:
...
- in any other case the customer is redirected to the `completeUrl` with **payment_id** and **subscription_id** query parameters. Use [get subscription](/docs/apis/rest/subscriptions-get/) endpoint to get the subscription status.

### 6. An asynchronous request is sent to your server

MONEI will notify you about the payment status by sending an HTTP POST request to the `callbackUrl`. The request body will contain the full [subscription object](/docs/apis/rest/schemas/subscription/) in JSON format.
