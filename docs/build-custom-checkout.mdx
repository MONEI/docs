...
description: Use MONEI UI components to build your custom checkout and securely accept Credit Cards, PayPal, Bizum, GooglePay and others payment methods.
---

Build you custom checkout using [MONEI UI components](monei-js-overview.mdx) to securely collect payments using different payment methods.

![MONEI Payments Demo](/img/payments-demo.jpg)
...
This page explains how to add different payment methods to your custom payment page. If you don't need a custom checkout experience we recommend using our [prebuilt payment page](use-prebuilt-payment-page.mdx). It already supports all available payment methods and does not require coding.

To test your integration:

- Use your [test mode](testing.md) Account ID and API Key.
- Make sure you have all payment methods configured. Check [Payment Methods](/payment-methods/card.mdx) section for more details about each payment method.
...
### 2. Add UI Component to your payment page `Client-side`
...
If you have a checkout flow where you need to generate payment token before you create a payment on your server you can initialise MONEI UI Components providing following parameters:
...
Check [Payment Methods](/payment-methods/card.mdx) section for more details about how to initialize components for each payment method.
...
You need to provide a `paymentId` (obtained in [step 1](#1-create-a-payment-server-side)) and `paymentToken` generated with UI Component. Check [Payment Methods](/payment-methods/card.mdx) section for more details about how to generate `paymentToken` for each payment method.
...
After you confirm the payment MONEI will automatically show a popup window with 3d secure confirmation screen (if needed, depending on the payment method)
...
- Make sure that you have a [payment method configured and enabled](https://dashboard.monei.com/settings/payment-methods) in live mode.
