...
Build your custom checkout using [MONEI UI Components](monei-js-overview.mdx) to securely collect payments using different payment methods.
...
If you have a checkout flow that requires you to generate a payment token before you create a payment on your server, you can initialize MONEI UI Components by providing the following parameters:
...
You need to provide a `paymentId` (obtained in [step 1](#1-create-a-payment-server-side)) and `paymentToken` generated with UI Component. Check the [Payment Methods](/payment-methods/card.mdx) section for more details about how to generate `paymentToken` for each payment method.
